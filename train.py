...
def training(args):
    ...
    # n_iterp = block_size + 2（block_size个伪帧 + 首末2个条件帧）
    # 与 args.block_size 保持一致，每个测试块生成与块内帧数相同数量的伪帧
    block_size = args.block_size  # 测试块内帧数，如 block_size=2 对应帧[9,10]
    n_iterp = block_size + 2      # 如 block_size=2 → n_iterp=4，含首末条件帧
    ...
    if args.lambda_vd_smooth>0:
        ...
        # pseudo_idx：从 DynamiCrafter 生成的 num_interpolate 帧中，
        # 按测试块内各帧相对于左右边界的真实时间位置采样。
        #
        # 测试块帧相对位置：帧 block_start+k 在 [block_start-1, block_end+1] 中的比例
        # 左边界帧(条件帧) = block_start - 1，右边界帧(条件帧) = block_end + 1
        # 块内第 k 帧（k=0..block_size-1）的相对位置 = (k+1) / (block_size+1)
        # 对应 DynamiCrafter 帧索引 = round(relative_pos * (num_interpolate-1))
        #
        # 例：block_size=2, num_interpolate=16
        #   k=0: pos=1/3 → t=round(5.0)=5
        #   k=1: pos=2/3 → t=round(10.0)=10
        # 首末加上条件帧 t=0 和 t=15，pseudo_idx=[0, 5, 10, 15]，n_iterp=4

        inner_ts = [
            round((k + 1) / (block_size + 1) * (dc.num_interpolate - 1))
            for k in range(block_size)
        ]
        pseudo_idx = np.array([0] + inner_ts + [dc.num_interpolate - 1])
        ...
        ## 保存伪帧（与训练完全一致：保存所有中间伪帧，共 block_size 帧）
        pseudo_save_dir = os.path.join(args.model_path, "pseudo_frames")
        os.makedirs(pseudo_save_dir, exist_ok=True)
        pseudo_cams_for_save = scene.getPseudoCameras(scale=scene.resolution_scales[-1],
                                                       num_interpolate=n_iterp,
                                                       cam_num=args.cam_num)
        for idx, (pseudo_img, cam) in enumerate(zip(pseudo_imgs, pseudo_cams_for_save)):
            # pseudo_img shape: (1, 1, C, n_iterp, H, W)
            # 保存所有中间伪帧（去掉首末条件帧），共 block_size 帧
            left_name  = cam["leftcam"].image_name
            right_name = cam["rightcam"].image_name
            for k in range(1, n_iterp - 1):  # k=1..block_size
                frame = pseudo_img[0, 0, :, k, :, :]          # (C, H, W)，值域 [-1, 1]
                frame = (frame.clamp(-1, 1) + 1) / 2          # 转到 [0, 1]
                save_image(frame, os.path.join(pseudo_save_dir,
                                               f"{left_name}_{right_name}_pseudo_k{k}.png"))
    ...
    for iteration in progress_bar:       
        ...
        if args.lambda_vd_smooth>0 and iteration%4==0: 
                ...
                # 取所有中间伪帧（去掉首末条件帧 t=0 和 t=-1），与渲染的 pseudo_images 对应
                # pseudo_img shape: (1, C, n_iterp, H, W)
                # 中间帧索引: [1, ..., n_iterp-2]，共 block_size 帧
                batch_gt = batch_gt[:, :, 1:-1, ...]  # (1, C, block_size, H, W)
