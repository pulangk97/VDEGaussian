...
from main.adapter import LoRALinearLayer, freeze_original_parameters
from lvdm.modules.attention import CrossAttention
# from submodules.DynamiCrafter.lvdm.modules.attention import CrossAttention
...
def load_imgs_iter(data_dir, iter_imgs_num, video_size = (256,256)):
        ...
        for i in range(N_imgs):
            avai_dir.append(f"r_{i}.png")

        # transform = transforms.Compose([transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),spatial_transform]) #outptu tensor in [-1,1]

        im = []
        for dir_ in avai_dir:

            img_tensor = torch.tensor(np.array(Image.open(os.path.join(data_dir, dir_))),dtype=torch.float32)[...,:3].permute(2,0,1)/255.

            im.append(transform(img_tensor))
        ...
        pair_idx = torch.tensor(np.stack(get_all_train_pairs(num_allimgs, test_idx)),dtype=int)

        im = torch.stack(im)

        return im[pair_idx[iter_imgs_num]]


def add_lora_to_ldmvfi(model, r=8, lora_alpha=32, replace_layer = None, adapte_layer = None):
    target_dtype = model.dtype if hasattr(model, 'dtype') else torch.float32
    # target_dtype = torch.float16

    for name, module in model.named_modules():
        if isinstance(module, replace_layer):

            module.to_q = adapte_layer(module.to_q, r, lora_alpha,device=model.device).to(target_dtype)
            module.to_k = adapte_layer(module.to_k, r, lora_alpha,device=model.device).to(target_dtype)
            module.to_v = adapte_layer(module.to_v, r, lora_alpha,device=model.device).to(target_dtype)
...
def load_data_prompts(data_dir, video_size=(256,256), video_frames=16, interp=False, imgs_iter = -1):
    transform = transforms.Compose([
        transforms.Resize(min(video_size)),
        transforms.CenterCrop(video_size),
        transforms.ToTensor(),
    ...
    ## load video
    file_list = get_filelist(data_dir, ['jpg', 'png', 'jpeg', 'JPEG', 'PNG', "JPG"])
    ...
    # load_imgs_iter(data_dir,video_size = (256,256))
    for idx in range(n_samples):
        ...
        if interp:
            ...
            frame_tensor1 = repeat(image_tensor1, 'c t h w -> c (repeat t) h w', repeat=np.floor(video_frames/2).astype(int))
            frame_tensor2 = repeat(image_tensor2, 'c t h w -> c (repeat t) h w', repeat=np.ceil(video_frames/2).astype(int))
        ...
        # print(torch.min(frame_tensor))
        # print(torch.max(frame_tensor))
...
def run_inference(args, gpu_num, gpu_no):
    ...
    # if args.iter_imgs_num>=0:
    #     source_path = "/media/xyr/data22/datasets/datasets/nerf_synthetic/lego/test"


    #     imgs = load_imgs_iter(source_path, torch.tensor(args.iter_imgs_num,dtype=int), video_size = (args.height, args.width))
    #     frame_tensor1 = repeat(imgs[0].unsqueeze(1), 'c t h w -> c (repeat t) h w', repeat=np.floor(n_frames/2).astype(int))
    #     frame_tensor2 = repeat(imgs[1].unsqueeze(1), 'c t h w -> c (repeat t) h w', repeat=np.ceil(n_frames/2).astype(int))
    #     frame_tensor = torch.cat([frame_tensor1, frame_tensor2], dim=1)
    ...
    # print(filename_list_rank)
    # assert False, print("debug")
    # print()

    start = time.time()
    with torch.no_grad():#, torch.cuda.amp.autocast():#torch.cuda.amp.autocast()
        for idx, indice in tqdm(enumerate(range(0, len(prompt_list_rank), args.bs)), desc='Sample Batch'):
            ...
            print(args.ddim_steps)
            print(args.ddim_eta)
            print(args.unconditional_guidance_scale)
            print(args.cfg_img)
            print(args.frame_stride)
            print(args.text_input)
            print(args.multiple_cond_cfg)
            print(args.loop)
            print(args.interp)
            print(args.timestep_spacing)
            print(args.guidance_rescale)
            ...
            for nn, samples in enumerate(batch_samples):
                ...
                if args.iter_imgs_num>=0:
                    filename = "length_16_noadapter"+ str(args.iter_imgs_num)+ filename # length_4_noadapter
                    # print(filename)
                # print(filename)
...
def get_parser():
    ...
    parser.add_argument("--adapter", action='store_true', default=False, help="use adapter or not")
    parser.add_argument("--iter_imgs_num", type=int, default=-1, help="num_iter")
